...
SELECT
    COUNT(DISTINCT o.customer_id) AS OrphanCustomerCount
...
SELECT
    COUNT(DISTINCT p.order_id) AS OrphanOrderCount
...
SELECT
    MIN(payment_date) AS MinPaymentDate,
    MAX(payment_date) AS MaxPaymentDate
FROM dbo.stg_payments;
GO
...
FROM dbo.stg_payments
GROUP BY
    LOWER(LTRIM(RTRIM(payment_status)))
...
SELECT
    COUNT(*) AS InvalidOrderAmountRows
FROM dbo.stg_orders
WHERE revenue < 0;
GO
...
SELECT
    COUNT(*) AS InvalidPaymentAmountRows
FROM dbo.stg_payments
WHERE amount < 0;
GO
...
SELECT
    COUNT(DISTINCT order_id) AS CancelledOrders
FROM dbo.stg_orders
...
SELECT
    COUNT(
        DISTINCT
        CASE
            WHEN LOWER(LTRIM(RTRIM(order_status))) = 'cancelled'
            THEN order_id
        END
    ) AS CancelledOrders,
...
SELECT
    COUNT(DISTINCT customer_id) AS CustomersWithOrders
FROM dbo.stg_orders
...
/* ============================================================
   ...
   It does NOT automatically define the
   2,389 At-Risk Customers.

   At-Risk classification will be handled separately
   using multiple behavioural signals.
   ============================================================ */
...
(
    SELECT
        MAX(order_date) AS AnalysisDate
    FROM dbo.stg_orders
)
SELECT
...
/*
    ...
    It must be calculated from the actual business definition
    established in the revenue-loss analysis.

    At-Risk Customers are NOT defined as 90-day churn customers.

    At-Risk logic must combine:
    ...
    The final 2,389 count must be produced by that
    segmentation logic.
